...
class dd_thread(threading.Thread):
    ...
    def read_process(self, count):
        ...
        dd_cmd = 'adb shell time sh -c "/data/busybox dd if=/dev/block/mmcblk0 of=/dev/null bs=1M count=' + str(count) + '"'
        #print 'exec {' + dd_cmd + '}'
    ...
    def write_process(self, count):
        ...
        #os.system(self.drop_cache_cmd)
        dd_cmd = 'adb shell time sh -c "/data/busybox dd if=/dev/zero of=' + self.ddtest + ' bs=10M count=' + str(count) + ' conv=fsync"'
        #print 'exec {' + dd_cmd + '}'
